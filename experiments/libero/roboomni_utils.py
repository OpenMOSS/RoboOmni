...
import numpy as np
import PIL.Image
from transformers import AutoProcessor, Qwen2_5OmniThinkerForConditionalGeneration, GenerationConfig
...
class RoboOmni:
    ...
    def __init__(
        ...
        Args:
            model_path (str): Hugging Face model ID or local path for the main
                            Qwen 2.5 Omni model and processor.
    ...
    def inference(self, image: np.ndarray, task_item: dict, unnorm_key: str = None,unnormalizer=None) -> np.ndarray:
        """
        ...
        speech_conv = task_item["speech_conv"]
        conv = task_item["conv"]
        ...
        if self.action_mode:
            ...
            messages.append({
                ...
                "content": [
                    ...
                    {"type": "audio", "audio": os.path.join(AUDIO_ROOT_DIR, speech_conv[len(self.history)].lstrip("./")) }
                ]
            })
        ...
        # print(generated_ids)
        output_ids = generated_ids[:, inputs["input_ids"].size(1):]
        response = self.processor.batch_decode(output_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
        ...
        # The formula is: unnormalized = 0.5 * (normalized + 1) * (high - low) + low
            ...
            self.history.append({"image": image, "response": response})
            
            if '[ACT]' in response:
                self.action_mode = True
            elif len(self.history) >= len(task_item):
