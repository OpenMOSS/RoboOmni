...
def get_args():
    ...
    p.add_argument("--data_root_dir", type=str, default="DATA_DIR")
    ...
    p.add_argument("--wandb_project_name", type=str, default="RoboOmni")
...
class SystemPromptFilter(logging.Filter):
    def filter(self, record):
        if "System prompt modified, audio output may not work" in record.getMessage():
            return False
        return True
...
def process_example(example: Dict[str, Any], fast_tokenizer: AutoProcessor) -> Dict[str, Any]:
    ...
    lang = example['lang']
    first_image = example['first_image']
